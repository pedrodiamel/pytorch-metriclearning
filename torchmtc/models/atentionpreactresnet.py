    ...
    def __init__(self, filtersin, filtersout, kernel_size=(3, 3), s=1, pad=0, is_batchnorm=False):
        ...
        if is_batchnorm:
            self.conv = nn.Sequential(
                nn.Conv2d(filtersin, filtersout, kernel_size, s, pad),
                nn.BatchNorm2d(filtersout),
                nn.ReLU(),
            )
        else:
            self.conv = nn.Sequential(
...
class AtentionResNet(nn.Module):
    ...
    def __init__(
        ...
        self.dec4 = DecoderBlockV2(
            bottom_channel_nr // 2 + num_filters * 8, num_filters * 8 * 2, num_filters * 8, is_deconv
        )
        self.dec3 = DecoderBlockV2(
            bottom_channel_nr // 4 + num_filters * 8, num_filters * 4 * 2, num_filters * 2, is_deconv
        )
        self.dec2 = DecoderBlockV2(
            bottom_channel_nr // 8 + num_filters * 2, num_filters * 2 * 2, num_filters * 2 * 2, is_deconv
        )
        self.dec1 = DecoderBlockV2(num_filters * 2 * 2, num_filters * 2 * 2, num_filters, is_deconv)

        # autoendocer
        self.autoencoder = nn.Sequential(
            ConvRelu(num_filters, num_filters), nn.Conv2d(num_filters, num_channels, kernel_size=1)
        )

        # feature
        self.conv_input = nn.Conv2d(
            in_channels=num_channels, out_channels=num_filters, kernel_size=9, stride=1, padding=4, bias=True
        )
        self.feature = self.make_layer(_Residual_Block_SR, 8, num_filters)
        self.conv_mid = nn.Conv2d(
            in_channels=num_filters, out_channels=num_filters, kernel_size=3, stride=1, padding=1, bias=True
        )

        ##############Attention Model#####################
        self.attention_model = nn.Sequential(
            nn.Conv2d(
                in_channels=num_filters + num_channels,
                out_channels=num_filters,
                kernel_size=3,
                stride=1,
                padding=1,
                bias=True,
            ),  # num_filters*2 + num_channels
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=num_filters, out_channels=num_filters, kernel_size=1, stride=1, padding=0, bias=True),
            # nn.Conv2d(in_channels=num_channels, out_channels=num_channels, kernel_size=1, stride=1, padding=0, bias=True),
        )

        self.rec1 = ConvRelu(num_filters, num_filters)
        self.rec2 = nn.Conv2d(num_filters, num_channels, kernel_size=1)

        # self.dec0 = ConvRelu(num_channels, num_channels)
    ...
    def forward(self, x):
        ...
        # att_in = torch.cat((att_in1, att_in2), dim=1)
        att_in = torch.cat((xr, att_in1), dim=1)
        # att_in = fmap

        scoremap = self.attention_model(att_in)

        fmap = torch.mul(fmap, scoremap)
        # detail_ft = torch.mul(srf, scoremap)
        # detail_ft = torch.mul(x, scoremap)
        # detail_ft = fmap

        # srf = torch.add(srf, fmap)
        srf = fmap
        ...
        conv6 = self.conv6(rec)  # + conv1
        conv7 = self.conv7(conv6)  # + conv2
        conv8 = self.conv8(conv7)  # + conv3
        conv9 = self.conv9(conv8)  # + conv4
        conv10 = self.conv10(conv9)  # + conv5

        # conv10 = torch.mul(conv10, conv5)
        # conv10 = conv10 + conv5
