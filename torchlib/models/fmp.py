...
__all__ = ['FMPNet', 'fmp', 'FMPEMBNet', 'fmpemb']
...
def fmpemb(pretrained=False, **kwargs):
    r"""FMPEmb model architecture
    https://www.andrew.cmu.edu/user/yzhiding/publications/ICMI15.pdf
    """
    model = FMPEMBNet(**kwargs)
    if pretrained:
        pass
        #model.load_state_dict(model_zoo.load_url(model_urls['fmp']))
    return model
...
class FMPEMBNet(nn.Module):
    
    def __init__(self, dim=64, num_channels=1, init_weights=True, batch_norm=False):
        super(FMPEMBNet, self).__init__()
        self.dim = dim
        self.num_channels = num_channels
        self.size_input=48 #48
